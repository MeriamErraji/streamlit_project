...
if __name__ == "__main__":
    ...
    # Maximum Drawdown
    p = np.maximum.accumulate(indicateur_ptf['Ptf_value'].dropna())
    Daily_Drawdown = indicateur_ptf['Ptf_value']/p - 1.0
    end = Daily_Drawdown.idxmin()
    
    max_drawdown_stats = {
        "MaxDD": Daily_Drawdown.min(),
        "start": indicateur_ptf['Ptf_value'].loc[:end].idxmax(),
        "end": end
    }
...
ax3_bis.plot(indicateur_ptf.index, Daily_Drawdown, 'm--', label="Drawdown")
...
st.title("📊 Visualisation des résultats du portefeuille")
...
# Maximum Drawdown
p = np.maximum.accumulate(indicateurs_offensifs['Valeur_Ptf'].dropna())
Daily_Drawdown = indicateurs_offensifs['Valeur_Ptf']/p - 1.0
end = Daily_Drawdown.idxmin()

max_drawdown_stats = {
    "MaxDD": Daily_Drawdown.min(),
    "start": indicateurs_offensifs['Valeur_Ptf'].loc[:end].idxmax(),
    "end": end
}
...
ax3_bis.plot(Daily_Drawdown.index, Daily_Drawdown, 'm--', label="Drawdown")
